...
def run_logistic_regression(hyperparameters):
    ...
    weights = np.concatenate((np.random.randn(M, 1), np.zeros(shape=(1, 1))), axis=0)
...
if __name__ == '__main__':
    ...
    hyperparameters = {
        'learning_rate': 0.5,   #0.1 FOR T_SMALL, 0.5 FOR TRAIN
        'weight_regularization': True,  # boolean, True for using Gaussian prior on weights
        'num_iterations': 200, #50 for small, 200 train
        'weight_decay': 1  # related to standard deviation of weight prior 0.01 FOR TRAIN, 1 FOR T_SMALL
    }
    ...
    # Cross Entropy
    iter_index = range(0, 200)
    plot.figure(1)
    plot.plot(logging[:, 1], color="blue", linewidth=2.5, linestyle="-", label="mnist_train")
    plot.plot(logging[:, 3], color="red", linewidth=2.5, linestyle="-", label="mnist_test")
    plot.legend(loc='upper right')
    plot.xlabel('Iteration')
    plot.ylabel('Cross Entropy')
    plot.title('Pen Cross Entropy Variation in test (a=0.01)')
    plot.grid(True)
    plot.show()
    ...
    iter_index =range(0,200)
    plot.figure(2)
    plot.plot(logging[:, 2], color="blue", linewidth=2.5, linestyle="-", label="mnist_train")
    plot.plot(logging[:, 4], color="red", linewidth=2.5, linestyle="-", label="mnist_test")
    plot.legend(loc='lower right')
    plot.xlabel('Iteration')
    plot.ylabel('Fraction Correct')
    plot.title('Pen Classification Error on test (a=0.01)')
    plot.grid(True)
    plot.show()
    # plot.axis([1, 9, 0.93, 1])
