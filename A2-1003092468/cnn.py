...
def InitCNN(num_channels, filter_size, num_filters_1, num_filters_2,
    ...
    #for momentum
    v_W1 = np.zeros(shape=(filter_size, filter_size,
                               num_channels,  num_filters_1))
    v_W2 = np.zeros(shape=(filter_size, filter_size,
                               num_filters_1, num_filters_2))
    v_W3 = np.zeros(shape=(num_filters_2 * 64, num_outputs))
...
def Conv2DBackward(grad_y, x, y, w):
    ...
    i = len(w) #rows
    j = len(w[0]) #columns
    w_T = np.transpose(w, [0,1,3,2])
    w_T_flip = w_T[::-1, ::-1]

    grad_x = Conv2D(grad_y, w_T_flip, pad = (i-1, j-1)) #transpose [3,2,0,1]
...
def showFilters(f):
    plt.figure(figsize=(1.0 * f.shape[3], 1.0))
    for i in xrange(f.shape[3]):
        plt.subplot(1, f.shape[3], i + 1)
        plt.imshow(f[:, :, 0, i], cmap=plt.cm.gray, interpolation="none")
        plt.tick_params(which='both')
    plt.savefig('CNN_filters__')
    plt.draw()
    plt.close()
...
def main():
    ...
    # Q3.4
    #print('Model Size:')
    #print(model['W1'].size + model['W2'].size + model['W3'].size + model['b1']
    #      .size + model['b2'].size + model['b3'].size)
    #showFilters(model['W1'])
    #print('Saving First Layer Filters...')
    ...
    #Q3.5
    train_data, valid_data, test_data, train_targets, valid_targets, test_targets = LoadData('../toronto_face.npz')
    x=valid_data
    t = valid_targets
    v = CNNForward(model, x)
    pred = Softmax(v['y'])
    im = x.reshape(x.shape[0], 48, 48)
    pred_class = np.max(pred, axis=1).reshape([-1, 1])
    thresh = 0.5
    num = 6
    count = 1
    plt.figure(2)
    for i in range(pred.shape[0]):
        if pred_class[i] < thresh:
            tit = "Target=%i\n Prediction=%i" % (np.argmax(t[i]), pred_class[i])
            print(tit)
            plt.tick_params(which='both', left='off', right='off', bottom='off')
            plt.subplot(2,3,count)
            plt.title(tit)
            plt.imshow(im[i, :, :], cmap=plt.cm.copper)
            plt.draw()

            count+=1
        if count > num:
            break
    plt.savefig('CNN_Uncert')
    plt.close()
