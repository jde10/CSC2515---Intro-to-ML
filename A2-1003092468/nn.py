...
def InitNN(num_inputs, num_hiddens, num_outputs):
    ...
    #for momentum
    v_W1 = np.zeros(shape=(num_inputs, num_hiddens[0]))
    v_W2 = np.zeros(shape=(num_hiddens[0], num_hiddens[1]))
    v_W3 = np.zeros(shape=(num_hiddens[1], num_outputs))
...
def AffineBackward(grad_y, x, w):
    ...
    grad_b = np.dot(grad_y.T, np.ones(x.shape[0]))
...
def CheckGrad(model, forward, backward, name, x):
    ...
    np.testing.assert_almost_equal(grad_w[check_elem], grad_w_2[check_elem],
                                   decimal=3)
def showWeights(w):
    plt.figure(figsize=(1.0 * w.shape[1], 1.0))
    for i in xrange(w.shape[1]):
        plt.subplot(1, w.shape[1], i + 1)
        plt.imshow(w[:, i].reshape(48, 48), cmap=plt.cm.gray)
        plt.tick_params(which='both')
    plt.savefig('NN_weights__')
    plt.draw()
    plt.close()
...
def main():
    ...
    print('Saving the first layer of weights...')
