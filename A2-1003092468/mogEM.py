...
def mogEM(x, K, iters, randConst=1, minVary=0):
    ...
    #------------------- Answers ---------------------
    #KMeans(Data matrix, Clusters, iters)
    #Clusters = Components = 7
    #iters = 5
...
def q2():
    ...
    #------------------- Answers ---------------------
    mixCoef, mean, variance, llhood = mogEM(inputs_train, K, iters, randConst=randConst, minVary=minVary)
    # mean vector -- mu
    ShowMeans(mean, 1)
    # variance vector -- vary
    ShowMeans(variance, 2)
    #mixing coefficients
    print 'mixing coefficients = %s' % mixCoef
    #log-likelihood
    plt.figure(3)
    plt.xlabel('Iteration')
    plt.ylabel('Log-Likelihood')
    plt.plot(llhood)
    plt.savefig('GraphLLhood_Q2.png')
...
def q4():
    ...
    trainingError = np.zeros(T)
    testingError = np.zeros(T)
    validationError = np.zeros(T)
    ...
    for t in xrange(T):
        ...
        #------------------- Answers ---------------------
        mixCoef_Anger, mean_Anger, variance_Anger, llhood_Anger = mogEM(x_train_anger, K, iters, randConst = randConst, minVary = minVary)

        mixCoef_Happy, mean_Happy, variance_Happy, llhood_Happy = mogEM(x_train_happy, K, iters, randConst = randConst, minVary = minVary)
        ...
        #-------------------- Add your code here ------------------------------
        #For Training
        llhood_train_anger = mogLogLikelihood(mixCoef_Anger, mean_Anger, variance_Anger, x_train) + log_likelihood_class[0]
        llhood_train_happy = mogLogLikelihood(mixCoef_Happy, mean_Happy, variance_Happy, x_train) + + log_likelihood_class[1]

        pred_train_labels = (llhood_train_anger < llhood_train_happy).astype(float)

        trainingError[t] = np.sum((pred_train_labels != y_train).astype(float)) / y_train.shape[0]
        print ('Training error: %.5f' % trainingError[t])

        #For Validation
        llhood_val_anger = mogLogLikelihood(mixCoef_Anger, mean_Anger, variance_Anger, x_valid) + log_likelihood_class[0]
        llhood_val_happy = mogLogLikelihood(mixCoef_Happy, mean_Happy, variance_Happy, x_valid) + + log_likelihood_class[1]

        pred_val_labels = (llhood_val_anger < llhood_val_happy).astype(float)

        validationError[t] = np.sum((pred_val_labels != y_valid).astype(float)) / y_valid.shape[0]
        print ('Validation error: %.5f' % validationError[t])

        #For Test
        llhood_test_anger = mogLogLikelihood(mixCoef_Anger, mean_Anger, variance_Anger, x_test) + log_likelihood_class[0]
        llhood_test_happy = mogLogLikelihood(mixCoef_Happy, mean_Happy, variance_Happy, x_test) + + log_likelihood_class[1]

        pred_test_labels = (llhood_test_anger < llhood_test_happy).astype(float)

        testingError[t] = np.sum((pred_test_labels != y_test).astype(float)) / y_test.shape[0]
        print ('Testing error: %.5f' % testingError[t])
    ...
    plt.plot(numComponents, trainingError, 'b', label='Training')
    plt.plot(numComponents, validationError, 'r', label='Validation')
    plt.plot(numComponents, testingError, 'g', label='Testing')
    plt.xlabel('Mixture Components')
    plt.ylabel('Classification Error Rate')
    plt.legend()
    plt.draw()
    plt.savefig('Graph Classification Error')
    plt.pause(0.0001)
